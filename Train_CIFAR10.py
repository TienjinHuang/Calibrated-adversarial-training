...
def kl_loss(output_robust,output_nat):
    kl = nn.KLDivLoss(reduction="sum")
    batch_size=output_nat.shape[0]    
    loss=(1.0 / batch_size) * kl(F.log_softmax(output_robust, dim=1),F.softmax(output_nat, dim=1))
    return loss
...
def get_args():
    ...
    parser.add_argument('--fname', default='cifar_model_b_5_b1_005_e8_resNet', type=str)
